//! Table, table-in-out, and config functions exposed by the symbols worker.
...
/// Register every table / table-in-out / config function on the worker.
pub fn register(worker: &mut Worker) {
    // The LATERAL / batch resolve surface (per-row column inputs → many rows).
    worker.register_table_in_out(resolve::Resolve);
    worker.register_table_in_out(resolve_batch::ResolveBatch);
